...
namespace Parcs.Portal.Services
{
    public class HostClient : IHostClient
    {
        ...
        {
            var requestUrl = string.Format(_hostConfiguration.GetModuleEndpoint, id);
            using var flurlClient = new FlurlClient(_httpClientFactory.CreateClient());
            return flurlClient.Request(requestUrl).GetJsonAsync<GetModuleHostResponse>(cancellationToken);
        }

        public Task<IEnumerable<GetModuleHostResponse>> GetModulesAsync(CancellationToken cancellationToken = default)
        {
            var requestUrl = _hostConfiguration.GetModulesEndpoint;
            using var flurlClient = new FlurlClient(_httpClientFactory.CreateClient());
            return flurlClient.Request(requestUrl).GetJsonAsync<IEnumerable<GetModuleHostResponse>>(cancellationToken);
        }
        ...
        public async Task<Result<CreateModuleHostResponse>> PostModuleAsync(CreateModuleHostRequest createModuleHostRequest, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var flurlClient = new FlurlClient(_httpClientFactory.CreateClient());
                var response = await flurlClient.Request(_hostConfiguration.PostModulesEndpoint).PostMultipartAsync(MultipartContent, cancellationToken);
        ...
        public async Task<Result<CreateJobHostResponse>> PostJobAsync(CreateJobHostRequest createJobHostRequest, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var flurlClient = new FlurlClient(_httpClientFactory.CreateClient());
                var response = await flurlClient.Request(_hostConfiguration.PostJobsEndpoint).PostMultipartAsync(MultipartContent, cancellationToken);
        ...
        {
            var requestUrl = _hostConfiguration.PostAsynchronousRunsEndpoint;

            using var flurlClient = new FlurlClient(_httpClientFactory.CreateClient());
            var response = await flurlClient.Request(requestUrl).PostJsonAsync(runJobHostRequest, cancellationToken);
