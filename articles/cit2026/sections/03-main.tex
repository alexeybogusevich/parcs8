...
\citsection{PARCS-Agent: Architecture, Protocol, and Case Study}
...
\begin{enumerate}[leftmargin=1.5cm,topsep=2pt,itemsep=2pt]
  \item \textit{The agent provides only per-worker logic.} Fan-out across 20 workers, the barrier between layers, and the propagation of layer~1's aggregated output to layer~2 (via the agent passing layer~1's \texttt{layerId} as \texttt{previousLayerId}) are all platform concerns invisible to the agent.
