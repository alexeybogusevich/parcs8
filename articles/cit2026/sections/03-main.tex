...
The architecture has two characteristic properties. First, all parallelism is encapsulated below the MCP boundary, so that the agent only observes synchronous tool invocations. Second, bulk input data does not transit the protocol: the MCP server retrieves a referenced URL once into a shared volume and provides each worker with an identical on-disk path, eliminating the per-worker file replication that would otherwise occur.
