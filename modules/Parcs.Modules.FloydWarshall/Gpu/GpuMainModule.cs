...
{
    /// <summary>
    /// GPU-accelerated main module for Floyd-Warshall APSP.
    ///
    /// The orchestration logic is identical to <c>ParallelMainModule</c>; the only difference
    /// is that workers are dispatched as <see cref="GpuWorkerModule"/> instances so that the
    /// inner relaxation loop runs on the GPU of each daemon node.
    /// </summary>
        ...
        public async Task RunAsync(IModuleInfo moduleInfo, CancellationToken cancellationToken = default)
        {
            ...
            if (initialMatrix.Height % moduleOptions.PointsNumber != 0)
            {
                throw new ArgumentException(
                    $"Matrix size (now {initialMatrix.Height}) must be divisible by {moduleOptions.PointsNumber}");
            }
            ...
            for (int i = 0; i < moduleOptions.PointsNumber; ++i)
            {
                ...
                // Dispatch the GPU worker variant instead of the CPU parallel worker.
            ...
            await moduleInfo.OutputWriter.WriteToFileAsync(
                JsonSerializer.SerializeToUtf8Bytes(moduleOutput), moduleOptions.OutputFile);
